...
ALPHA=(8.,12.);WINDOW=(.2,1.2);BASELINE=(-.3,-.1)
FREQS=np.arange(2.,31.,1.);N_CYCLES=FREQS/2.;TIME_BANDWIDTH=2.;DECIM=2
ROI8=("PO3","POz","PO4","O1","Oz","O2","PO7","PO8");ROI3=("PO3","POz","PO4")
PTHRESH=.05

def args():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument("--subjects",nargs="+",required=True);p.add_argument("--session",default="01");p.add_argument("--task",default="SpAtt");p.add_argument("--run",default="01");p.add_argument("--platform",choices=["mac","bluebear"],default="mac");p.add_argument("--project-root",default=None);p.add_argument("--n-jobs",type=int,default=4);p.add_argument("--power-mode",choices=["percent","raw"],default="percent");p.add_argument("--seed",type=int,default=42);return p.parse_args()
def load(root,s,a):
 d={}
 for c in CONDITIONS:
...
def main():
 ...
 # audit
 audit={"subjects":subs,"n_subjects":len(subs),"common_good_channels":common,"alpha_hz":ALPHA,"window_s":WINDOW,"baseline_s":BASELINE if a.power_mode=="percent" else None,"power_mode":a.power_mode,"ROI8":roi_results.get("8-channel posterior/occipital ROI"),"ROI3":roi_results.get("3-channel posterior ROI"),"significant_sensorwise_fdr":sigsensor,"spatiotemporal":[{k:v for k,v in r.items() if k!="mask"} for r in srows]}
