...
STAGES=[(0,'posterior-alpha inclusion QC',PREPROCESSING/'P00_screen_subject.py'),(1,'LP100 + PyPREP/manual channel QC -> segmentation -> epochs -> optional rereference',PREPROCESSING/'P01_epoch_pyprep_reref.py'),(2,'ICA on concatenated condition epochs',PREPROCESSING/'P02_ica.py'),(3,'manual epoch rejection + spectral QC',PREPROCESSING/'P03_manual_epoch_rejection.py'),(4,'ERP analysis',SENSOR/'A01_ERP.py'),(5,'TFR analysis',SENSOR/'A02_TFR.py'),(6,'manuscript-style analysis overview',SENSOR/'A03_analysis_overview.py')]
def parse_args():
 ...
 p.add_argument('--session',default='01');p.add_argument('--task',default='SpAtt');p.add_argument('--run',default='01');p.add_argument('--platform',choices=['mac','bluebear'],default='mac');p.add_argument('--project-root',default=None);p.add_argument('--rereference',choices=['avg'],default=None,help='Omit for no rereferencing; use --rereference avg for average EEG reference.');p.add_argument('--from-stage',type=int,choices=range(7),default=0);p.add_argument('--continue-on-error',action='store_true');return p.parse_args()
