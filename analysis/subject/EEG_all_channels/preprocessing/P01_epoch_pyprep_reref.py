...
from pipeline_config import CONDITIONS,base_bids_path,crop_table_path,qc_dir,resolve_project_root,referenced_stage_path
...
def parse_args():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--subject',required=True);p.add_argument('--session',default='01');p.add_argument('--task',default='SpAtt');p.add_argument('--run',default='01');p.add_argument('--platform',choices=['mac','bluebear'],default='mac');p.add_argument('--project-root',default=None);p.add_argument('--rereference',choices=['avg','none'],default='avg',help='Default: avg. Use --rereference none to keep the original reference.');return p.parse_args()
...
def ask_reasons(channels):
 ...
 return out
def main():
 a=parse_args();s=a.subject.removeprefix('sub-');root=resolve_project_root(a.platform,a.project_root);report=participant_report(root,s);figs=figure_dir(root,s);do_avg=a.rereference=='avg';reref_label='average EEG reference' if do_avg else 'none';reref_arg='avg' if do_avg else None
 ...
 raw.filter(l_freq=None,h_freq=100.0);reasons,errors=pyprep_reasons(raw);suggested=sorted(reasons);raw.info['bads']=sorted(set(raw.info['bads'])|set(suggested))
 print('\nPyPREP suggested bad channels:',suggested or 'None')
 for ch,why in sorted(reasons.items()):print(f'  {ch}: {", ".join(why)}')
 before=set(raw.info['bads']);raw.plot(n_channels=min(30,len(raw.ch_names)),duration=20.,block=True,title=f'sub-{s}: post-PyPREP manual channel QC');after=set(raw.info['bads']);manual_additions=sorted(after-before);browser_rescues=sorted(before-after);manual_reasons=ask_reasons(manual_additions)
 rejected=sorted(after)
 if rejected:
  rr=raw.copy().pick(rejected);rr.info['bads']=[];spec=rr.compute_psd(fmin=.5,fmax=min(100.,rr.info['sfreq']/2.));f=spec.plot(show=False);report.add_figure(f,str(figs/'P01_rejected_channels_PSD.png'),'PSD of PyPREP + manually rejected channels',f'Channels proposed for rejection: {fmt_channels(rejected)}.','All-channel preprocessing');plt.close(f);spec.plot(show=True);plt.show(block=True);rescued=input('Rejected channels that are actually GOOD and should be rescued (space-separated, Enter for none): ').strip().split()
 else:rescued=[]
 unknown=[ch for ch in rescued if ch not in rejected]
 if unknown:raise ValueError(f'Can only rescue channels shown in rejected-channel PSD: {unknown}')
 final=sorted(set(rejected)-set(rescued));raw.info['bads']=final;print('FINAL bad channels:',final or 'None')
 good=[ch for ch in raw.copy().pick('eeg').ch_names if ch not in final]
 if not good:raise RuntimeError('No good EEG channels remain after channel QC.')
 gr=raw.copy().pick(good);gr.info['bads']=[];gs=gr.compute_psd(fmin=.5,fmax=min(100.,gr.info['sfreq']/2.));gf=gs.plot(show=False);report.add_figure(gf,str(figs/'P01_final_good_channels_PSD.png'),'Final PSD of remaining good EEG channels',f'Final retained EEG channels before segmentation: {fmt_channels(good)}.','All-channel preprocessing');plt.close(gf);print('\nOpening FINAL PSD of remaining GOOD EEG channels. Close to continue.');gs.plot(show=True);plt.show(block=True)
 table=json.loads(crop_table_path().read_text(encoding='utf-8'));key=f'sub-{s}'
 if key not in table:raise KeyError(f'No stimulation crop times for {key}')
 audit={'subject':key,'low_pass_hz':100,'rereference':a.rereference,'rereference_applied':do_avg,'pyprep_reasons':reasons,'detector_errors':errors,'pyprep_suggested_bad_channels':suggested,'manual_raw_browser_additions':manual_additions,'manual_raw_browser_reasons':manual_reasons,'raw_browser_rescued_pyprep_channels':browser_rescues,'rejected_channels_shown_in_psd':rejected,'channels_rescued_after_rejected_psd':rescued,'bad_channels':final,'final_good_eeg_channels':good,'conditions':{}}
 for condition in CONDITIONS:
  ...
  if do_avg:epochs.set_eeg_reference(ref_channels='average',projection=False)
  out=referenced_stage_path(root,s,a.session,a.task,a.run,condition,'epochs',reref_arg);epochs.save(out,overwrite=True);audit['conditions'][condition]={'crop_times_sec':times,'n_epochs':len(epochs),'rereference':reref_label,'output':str(out)};report.add_text(f'{condition}: stimulation segmentation',f'Kept ranges (s): {times}\nCue epochs: -0.5 to +1.6 s; baseline=None; detrend=1.\nRereferencing: {reref_label}.\nEpochs: {len(epochs)}','Stimulation segmentation')
 reason_text='\n'.join(f'{ch}: {", ".join(v)}' for ch,v in sorted(reasons.items())) or 'None';manual_text='\n'.join(f'{ch}: {manual_reasons[ch]}' for ch in manual_additions) or 'None';report.add_text('PyPREP, manual channel QC, and reference',f'PyPREP was run on continuous EEG after 100-Hz low-pass.\nPyPREP suggestions:\n{reason_text}\nUser-rejected channels and reasons:\n{manual_text}\nChannels rescued in raw browser: {fmt_channels(browser_rescues)}\nChannels rescued after rejected-channel PSD: {fmt_channels(rescued)}\nFINAL bad channels: {fmt_channels(final)}\nFinal good EEG channels: {fmt_channels(good)}\nRereferencing: {reref_label}. Average rereferencing is the default; use --rereference none to disable it.','All-channel preprocessing');(qc_dir(root,s)/'P01_pyprep_segment_epoch_reref.json').write_text(json.dumps(audit,indent=2)+'\n',encoding='utf-8')
