"""P02: ICA on concatenated stim + no-stim epochs.

One shared ICA is fitted on concatenated condition epochs using a 1-40 Hz, 200-Hz
fitting copy. Manual ICA QC is sequential: first inspect and close all component
scalp maps, then inspect and close the component source time-series browser, then
enter the components to exclude. The fitted solution is applied separately to the
original full-resolution stimulation and no-stimulation epochs.
"""
...
def parse_components(text,n):
 ...
 return vals
def main():
 a=parse_args();s=a.subject.removeprefix('sub-');root=resolve_project_root(a.platform,a.project_root);report=participant_report(root,s);figs=figure_dir(root,s);epochs={}
 for c in CONDITIONS:
 ...
 fit=mne.concatenate_epochs([epochs[c].copy() for c in CONDITIONS],add_offset=True,on_mismatch='warn');fit.pick('eeg');fit.resample(200);fit.filter(1,40);good=[ch for ch in fit.ch_names if ch not in fit.info['bads']];n=min(a.n_components,max(2,len(good)-1));print(f'Fitting one ICA with {n} components on {len(fit)} concatenated stim + no-stim epochs.');ica=ICA(method='fastica',random_state=a.random_state,n_components=n,max_iter='auto');ica.fit(fit,picks='eeg',reject_by_annotation=True)
 print('\nICA MANUAL QC - STEP 1/2: COMPONENT SCALP MAPS');print('Inspect all ICA component topographies. Close ALL component-map windows when finished to continue to the time series.')
 ...
 plt.show(block=True)
 print('\nICA MANUAL QC - STEP 2/2: COMPONENT TIME SERIES');print('Inspect the ICA source time series. Close the source browser when finished; you will then be asked which components to exclude.')
 ica.plot_sources(fit,block=True,title=f'sub-{s}: ICA component time series')
 while True:
 ...
 ica.exclude=excluded;ica.save(qc_dir(root,s)/f'sub-{s}_ica.fif',overwrite=True);audit={'subject':f'sub-{s}','fit_on':'concatenated stim + no-stim epochs','fit_resample_hz':200,'fit_filter_hz':[1,40],'n_components':int(ica.n_components_),'manual_qc_views':'sequential: component scalp maps first, then source time series, then exclusion input','excluded_components':excluded,'conditions':{}}
 for c in CONDITIONS:
 ...
 report.add_text('ICA method and decision',f'One shared ICA decomposition was fitted on concatenated stim + no-stim cue epochs.\nICA fitting copy: resampled to 200 Hz and filtered 1-40 Hz.\nMethod: FastICA; random state: {a.random_state}.\nComponents fitted: {ica.n_components_}.\nManual ICA QC was sequential: all component scalp maps were inspected first and closed; the component source time-series browser was then inspected and closed; component exclusions were entered only after both inspections.\nManually excluded components: {excluded or "None"}.\nThe fitted solution was then applied separately to the original stim epochs and no-stim epochs.\nBad channels retained in info: {fmt_channels(epochs[CONDITIONS[0]].info["bads"])}','ICA');(qc_dir(root,s)/'P02_ica_decisions.json').write_text(json.dumps(audit,indent=2)+'\n',encoding='utf-8')
