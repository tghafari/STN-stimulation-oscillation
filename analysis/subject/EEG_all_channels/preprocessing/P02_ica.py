...
fitting copy. During manual ICA QC, component scalp maps and component source time
series are opened at the SAME TIME. Keep both windows open and compare matching IC
numbers before entering exclusions in the terminal. The fitted solution is then
applied separately to the original full-resolution stim and no-stim epochs.
"""
...
def main():
    ...
    print(f'Fitting one ICA with {n} components on {len(fit)} concatenated stim + no-stim epochs.')
    ...
    # Open BOTH diagnostic views non-blocking so they remain visible together.
    component_figs=ica.plot_components(picks=range(ica.n_components_),show=True)
    component_figs=component_figs if isinstance(component_figs,(list,tuple)) else [component_figs]
    for i,fig in enumerate(component_figs):
        report.add_figure(fig,str(figs/f'P02_all_ICA_components_{i+1}.png'),f'All ICA components ({i+1}/{len(component_figs)})',f'All {ica.n_components_} fitted component topographies. No component is omitted.','ICA')
    source_fig=ica.plot_sources(fit,block=False,title=f'sub-{s}: ICA component time series - compare IC numbers with scalp maps')
    plt.show(block=False)
    print('\nICA MANUAL QC')
    print('Two diagnostic windows are now open at the same time:')
    print('  1) ICA component scalp topographies')
    print('  2) ICA component source time series')
    print('Compare the SAME IC number across the two windows before excluding it.')
    print('Keep both windows open while you enter your exclusion decision below.')

    while True:
        try:excluded=parse_components(input('ICA components to EXCLUDE after comparing maps + time series (space/comma separated, Enter for none): '),ica.n_components_);break
    ...
    audit={'subject':f'sub-{s}','fit_on':'concatenated stim + no-stim epochs','fit_resample_hz':200,'fit_filter_hz':[1,40],'n_components':int(ica.n_components_),'manual_qc_views':'component scalp maps and source time series displayed simultaneously','excluded_components':excluded,'conditions':{}}
    for c in CONDITIONS:
