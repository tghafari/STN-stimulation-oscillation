...
def choose(candidates):
 print('\nInspect the eight posterior/occipital TFR channel figures, then close them to continue.');print('Available candidates:',', '.join(candidates))
 while True:
...
def add(report,figs,t,stem,title,caption,candidates):
 lim=vlim(t);sf=scalp(t,lim);report.add_figure(sf,str(figs/f'{stem}_sensor_topography.png'),f'{title}: all sensors separately in scalp layout',caption+f' Shared robust scale: {lim}.','Time-frequency analysis');cf=separate(t,candidates,title,lim);report.add_figure(cf,str(figs/f'{stem}_ROI_candidates_separate.png'),f'{title}: eight posterior/occipital candidates separately',caption+f' Candidate channels displayed separately: {fmt_channels(candidates)}.','Time-frequency analysis');return lim,cf
def main():
 ...
 raw={c:compute(eps[c].copy().pick(common),a.n_jobs) for c in CONDITIONS};display={c:raw[c].copy().apply_baseline(BASELINE,mode='percent') for c in CONDITIONS};scales={};candidate_figs=[]
 for c in CONDITIONS:
  name='Stimulation' if c=='stim' else 'No stimulation';scales[c],cf=add(report,figs,display[c],f'A02_{c}_TFR',f'{name}: combined attention-left/right TFR',f'Percent baseline {BASELINE}; multitaper 2-30 Hz.',candidates);candidate_figs.append(cf)
 diff=raw['stim'].copy();diff.data=raw['stim'].data-raw['no-stim'].data;scales['difference'],cf=add(report,figs,diff,'A02_stim_minus_no_stim_TFR','TFR: stimulation - no stimulation','Unbaselined power; NO baseline correction.',candidates);candidate_figs.append(cf)
 ratio=raw['stim'].copy();ratio.data=(raw['stim'].data-raw['no-stim'].data)/(raw['stim'].data+raw['no-stim'].data+np.finfo(float).eps);scales['ratio'],cf=add(report,figs,ratio,'A02_stim_normalized_difference_TFR','TFR: (stim - no-stim) / (stim + no-stim)','Unbaselined power; NO baseline correction.',candidates);candidate_figs.append(cf)
 # Interactive ROI decision: show ONLY the four eight-channel TFR figures. The all-sensor scalp layouts remain report outputs only.
 for fig in candidate_figs:fig.show()
 plt.show(block=True)
 roi,excluded=choose(candidates)
 ...
 details={'subject':f'sub-{s}','method':'multitaper','frequencies_hz':FREQS.tolist(),'n_cycles':'frequency / 2','time_bandwidth':2,'decim':2,'condition_baseline_s':list(BASELINE),'condition_baseline_mode':'percent','comparative_baseline':None,'roi_candidates_predefined':list(ROI_CANDIDATES),'roi_candidates_available':candidates,'roi_excluded_by_user':excluded,'roi_final':roi,'roi_decision_view':'eight separate posterior/occipital TFR channels only for stim, no-stim, difference and ratio','vlims':scales};(qc_dir(root,s)/'A02_tfr_analysis.json').write_text(json.dumps(details,indent=2)+'\n');report.add_text('TFR analysis details',f'Multitaper power: 2-30 Hz in 1-Hz steps; n_cycles=frequency/2; time-bandwidth=2; FFT=True; ITC=False; trial average=True; decim=2. Stim/no-stim use percent baseline {BASELINE}; comparative TFRs use no baseline correction. Eight separate posterior/occipital channels: {fmt_channels(ROI_CANDIDATES)}. ROI exclusion decisions were made from the eight-channel TFR figures only.\nFinal channels contributing to ROI mean: {fmt_channels(roi)}.','Time-frequency analysis');print(f'TFR complete for sub-{s}.')
