...
def add(report,figs,t,stem,title,caption,candidates):
 lim=vlim(t);report.add_figure(scalp(t,lim),str(figs/f'{stem}_sensor_topography.png'),f'{title}: all sensors separately in scalp layout',caption+f' Shared robust scale: {lim}.','Time-frequency analysis');report.add_figure(separate(t,candidates,title,lim),str(figs/f'{stem}_ROI_candidates_separate.png'),f'{title}: eight posterior/occipital candidates separately',caption+f' Candidate channels displayed separately: {fmt_channels(candidates)}.','Time-frequency analysis');return lim
def main():
