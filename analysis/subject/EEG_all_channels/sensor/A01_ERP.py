"""A01: ERP analysis with eight-channel interactive ROI selection followed by full report plots."""
...
ROI_CANDIDATES=('PO3','POz','PO4','O1','Oz','O2','PO7','PO8');ERP_BASELINE=(-0.1,0.0);ERP_LP_HZ=30.;ERP_TMIN=-0.1;ERP_TMAX=0.5
def parse_args():
...
def eight_fig(compare,candidates):
 ...
 fig.suptitle('ERP ROI selection: stimulation vs no stimulation');return fig
def choose(candidates):
 print('\nInspect ONLY the eight ERP sensors shown, then close the figure.');print('ROI candidates:',', '.join(candidates))
 while True:
  ex=input('Channels to EXCLUDE from ROI mean (Enter for none/all eight included): ').replace(',',' ').split();bad=[x for x in ex if x not in candidates]
...
def main():
 ...
 ev={c:evoked(eps[c],common) for c in CONDITIONS};compare={'no stimulation':ev['no-stim'],'stimulation':ev['stim']};candidates=[ch for ch in ROI_CANDIDATES if ch in common]
 if not candidates:raise RuntimeError('None of the eight ROI candidates are good in both conditions.')
 # FIRST: only the eight candidate sensors are shown interactively. No all-channel plot is made before this decision.
 fig=eight_fig(compare,candidates);plt.show(block=True);roi,excluded=choose(candidates)
 if not roi:raise RuntimeError('ROI cannot be empty.')
 # AFTER ROI decision: generate report/result figures.
 report.add_figure(fig,str(figs/'A01_ERP_stim_vs_no_stim_ROI_candidates_separate.png'),'ERP: eight posterior/occipital ROI candidates separately',f'Candidate channels: {fmt_channels(candidates)}.','ERP analysis')
 ...
 fm=mne.viz.plot_compare_evokeds(compare,picks=roi,combine='mean',show=False,ci=False,truncate_xaxis=False,truncate_yaxis=False);fm=fm[0] if isinstance(fm,list) else fm;fm.axes[0].axvline(0,color='k',linestyle='--',linewidth=1);fm.axes[0].set_xlim(ERP_TMIN,ERP_TMAX);fm.axes[0].set_title(f'sub-{s}: posterior/occipital ROI mean');report.add_figure(fm,str(figs/'A01_ERP_stim_vs_no_stim_ROI_mean.png'),'ERP: posterior/occipital ROI mean',f'Final channels contributing to ROI mean: {fmt_channels(roi)}.','ERP analysis')
 for c in CONDITIONS:mne.write_evokeds(stage_path(root,s,a.session,a.task,a.run,c,'erp','ave'),ev[c],overwrite=True)
 details={'subject':f'sub-{s}','epoch_original_window_s':[-0.5,1.6],'erp_display_window_s':[ERP_TMIN,ERP_TMAX],'baseline_s':list(ERP_BASELINE),'evoked_low_pass_hz':ERP_LP_HZ,'roi_candidates_predefined':list(ROI_CANDIDATES),'roi_candidates_available':candidates,'roi_excluded_by_user':excluded,'roi_final':roi,'roi_decision_view':'eight ERP sensors only; all-channel scalp layout generated after ROI decision','common_good_eeg_sensors':common};(qc_dir(root,s)/'A01_erp_analysis.json').write_text(json.dumps(details,indent=2)+'\n');report.add_text('ERP analysis details',f'Attention-left/right trials combined. Trial-average ERP; 30-Hz low-pass; baseline -0.1 to 0 s; display -0.1 to 0.5 s. ROI selection was performed first using only {fmt_channels(candidates)}; all-sensor scalp layout was generated afterwards.\nFinal channels contributing to ROI mean: {fmt_channels(roi)}.','ERP analysis')
