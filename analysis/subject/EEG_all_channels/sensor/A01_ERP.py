...
1. one scalp-layout figure with a separate ERP trace at every common good EEG
   sensor, positioned according to the electrode montage;
2. PO3, PO4 and POz as three enlarged separate traces (when available);
3. one enlarged trace representing the arithmetic mean of available PO3/PO4/POz.

ERPs are trial averages, low-pass filtered at 30 Hz, baseline corrected from
-0.1 to 0 s, and displayed from -0.1 to 0.5 s relative to cue onset.
"""
...
def common_good_eeg(epochs):
    ...
    return [ch for ch in stim.copy().pick('eeg').ch_names if ch in nostim.ch_names and ch not in stim.info['bads'] and ch not in nostim.info['bads']]
...
def main():
    ...
    # 1) Every sensor gets its own trace, arranged at its scalp position.
    ...
    if posterior:
        # 2) Three posterior channels enlarged and separate.
        ...
        report.add_figure(fig_post,str(figs/'A01_ERP_stim_vs_no_stim_posterior_separate.png'),'ERP: PO3, PO4 and POz separately',f'Available predefined posterior sensors shown separately: {fmt_channels(posterior)}. Trial mean; {ERP_LP_HZ:g}-Hz low-pass; baseline {ERP_BASELINE}.','ERP analysis')

        # 3) Mean across the available predefined posterior ROI.
        ...
        report.add_figure(fig_mean,str(figs/'A01_ERP_stim_vs_no_stim_posterior_mean.png'),'ERP: mean of PO3, PO4 and POz',f'Arithmetic sensor mean across the available predefined posterior ROI: {fmt_channels(posterior)}. Stimulation and no-stimulation overlaid; trial mean; {ERP_LP_HZ:g}-Hz low-pass; baseline {ERP_BASELINE}.','ERP analysis')
    else:
        report.add_text('Posterior ERP unavailable','None of PO3, PO4 or POz was good in both conditions.','ERP analysis')

    details={'subject':f'sub-{subject}','epoch_original_window_s':[-0.5,1.6],'erp_display_window_s':[ERP_TMIN,ERP_TMAX],'baseline_s':list(ERP_BASELINE),'evoked_low_pass_hz':ERP_LP_HZ,'trial_averaging':'arithmetic mean','views':['separate sensor ERPs in scalp layout','PO3/PO4/POz separately','mean across available PO3/PO4/POz'],'common_good_eeg_sensors':common,'posterior_available':posterior,'n_epochs':{c:len(epochs[c]) for c in CONDITIONS}}
    (qc_dir(root,subject)/'A01_erp_analysis.json').write_text(json.dumps(details,indent=2)+'\n',encoding='utf-8')
    report.add_text('ERP analysis details',f'Input epochs: -0.5 to +1.6 s around cue onset; attention-left/right combined within condition.\nERP: arithmetic mean across retained trials; low-pass {ERP_LP_HZ:g} Hz; baseline {ERP_BASELINE[0]:g} to {ERP_BASELINE[1]:g} s; display {ERP_TMIN:g} to {ERP_TMAX:g} s.\nViews: every common good sensor separately in scalp layout; PO3/PO4/POz separately; and their sensor mean.\nCommon good sensors: {len(common)}. Posterior available: {fmt_channels(posterior)}.','ERP analysis')
