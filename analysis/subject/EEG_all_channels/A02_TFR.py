...
Multitaper power: 2-30 Hz, 1-Hz steps, n_cycles=f/2, time_bandwidth=2,
decim=2, average across retained trials. Condition-specific stim/no-stim displays
use percent baseline -0.3 to -0.1 s. Comparative maps (stim-no-stim and normalized
difference) are calculated from UNBASELINED power and receive NO baseline correction.

Every TFR result is shown three ways: all common good sensors separately in scalp
layout, PO3/PO4/POz separately, and the mean across available PO3/PO4/POz.
"""
...
POSTERIOR=('PO3','PO4','POz'); BASELINE=(-0.3,-0.1); FREQS=np.arange(2.,31.,1.); N_CYCLES=FREQS/2.; TIME_BANDWIDTH=2.; DECIM=2; PLOT_TMIN=-0.3; PLOT_TMAX=1.4
...
def add_three_views(report,figs,tfr,stem,title,caption,posterior):
    ...
    if posterior:
        report.add_figure(posterior_separate(tfr,posterior,title),str(figs/f'{stem}_posterior_separate.png'),f'{title}: PO3, PO4 and POz separately',caption+f' Available posterior sensors: {fmt_channels(posterior)}.','Time-frequency analysis')
        report.add_figure(posterior_mean(tfr,posterior,f'{title}: posterior mean'),str(figs/f'{stem}_posterior_mean.png'),f'{title}: mean of PO3, PO4 and POz',caption+f' Arithmetic sensor mean across: {fmt_channels(posterior)}.','Time-frequency analysis')
