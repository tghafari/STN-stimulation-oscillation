...
%% Basic Setup
clyear; close all; sca; fclose('all');  % this should be outside of function, otherwise might not work!
...
cfgTrigger = introduce_triggers;  % introduce triggers  %needs Benchi's response
...
cfgScreen = initialise_screen_variables(cfgExp);  % introduce visual variables  %needs Benchi's response
...
cfgEyelink = initialise_eyelink(cfgFile, cfgEyelink, cfgScreen, cfgExp);  % initialise eyelink  %needs Benchi's response
...
cfgTrigger = initialise_trigger_port(cfgExp, cfgTrigger);  % initiate triggers  %needs Benchi's response
%% Main Experiment

cfgExp = KbQueue_start_routine(cfgExp);  % start KbQueu routine  %needs Benchi's response
...
for blk = 1:cfgExp.numBlock
    cfgOutput.blkStrtTmPnt(blk) = send_trigger(cfgTrigger, cfgExp, cfgTrigger.blkStart, cfgEyelink, sprintf('block n. %d', blk));  %needs Benchi's response
