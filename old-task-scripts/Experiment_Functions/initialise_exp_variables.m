...
rng('shuffle')
% total time: ~44 minute without breaks (5 to 7.5 sec each trial, ~5.5 min each block)
cfgExp.numBlock = 8;  % total number of blocks (8)
cfgExp.numTrial = 60;  % number of trials in each block (60)
cfgExp.numStim = cfgExp.numTrial * cfgExp.numBlock;  % number of stimuli in total
cfgExp.ITIDur =  1000 + (1500 - 1000) .* rand(cfgExp.numStim,1);  % duration of ITI in ms (jitter between 1 and 1.5 sec)
cfgExp.cueDur = 200;  % duration of cue presentation in ms
cfgExp.ISIDur = 1000;  % interval between cue and grating (stimulus)
cfgExp.stimDur = 1000 + (2000 - 1000) .* rand(cfgExp.numStim,1);  % duration of visual stimulus in ms (jitter between 1 and 2 sec)
